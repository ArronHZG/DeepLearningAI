...
def model(X, Y, learning_rate=0.3, num_iterations=50000, print_cost=True, lambd=0, keep_prob=1):
    """
...
def showRegular():
    parameters = model(train_X, train_Y, print_cost=False, lambd=0.8)
...
def diffLamdb():
    ...
    lamdbs = [i/10 for i in range(1,11)]
    for lamdb in lamdbs:
        plt.subplot(5, 2, lamdb*10)
        plt.subplots_adjust(wspace=0.3, hspace=1)  # 调整子图间距
        plt.title('Lamda is  %f' % lamdb)
        parameters = model(train_X, train_Y, print_cost=False, lambd=lamdb)
        axes = plt.gca()
        axes.set_xlim([-0.75, 0.40])
        axes.set_ylim([-0.75, 0.65])
        plot_decision_boundary(lambda x: predict_dec(parameters, x.T), train_X, train_Y[0, :])
